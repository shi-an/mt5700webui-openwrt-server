...
1.  **`at-webserver` (Rust Backend)**
    *   高性能 WebSocket AT 命令服务器，支持单 IP 独立连接与防假死心跳拦截。
    ...
    *   **Flash 保护**：内置 8KB/5秒 日志缓冲与 1MB 自动轮转机制，延长路由器寿命。
    *   **智能短信管理**：支持仅在成功转发到第三方通道后自动删除短信。
...
2.  **`luci-app-at-webserver` (LuCI Frontend)**
    ...
    *   **自动日志路径**：根据持久化配置自动切换 `/tmp` (内存) 或 `/var/log` (持久化)。
